  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-br from-cream via-warmWhite to-cream">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold mb-4 text-darkBrown font-baloo">Notifications</h1>
        {(!notifications || notifications.length === 0) && (
          <div className="text-sm text-darkBrown/60">You have no notifications.</div>
        )}
        <div className="space-y-3 mt-4">
            ...
            return (
            <div key={id} className={`p-4 border-2 border-caramel/30 rounded-2xl bg-gradient-to-br from-cream to-warmWhite shadow-lg ${n.read ? 'opacity-60' : ''}`}>
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-darkBrown truncate">{expanded ? full : preview}</div>
                </div>
                <div className="ml-4 text-right">
                  <div className="text-xs text-darkBrown/60">{n.createdAt ? formatDate(n.createdAt) : ''}</div>
                  <div className="mt-2 text-xs text-darkBrown/60">{(!(n.adminConfirmation || n.adminNotification) && n.read) ? 'Read' : ''}</div>
                </div>
              </div>
              <div className="mt-2 flex items-center justify-between">
                {(() => {
                  ...
                  return (
                    <div className="text-xs text-darkBrown/60">{showPoints ? `Points: ${typeof n.rewardPointsAdded === 'number' ? `+${n.rewardPointsAdded}` : '-'} • Total: ${typeof n.totalPoints === 'number' ? n.totalPoints : '-'}` : ''}</div>
                  );
                })()}
                {long && (
                  <div>
                    <button className="text-xs text-caramel hover:text-darkBrown transition-colors" onClick={() => setExpandedIds(prev => ({ ...prev, [id]: !prev[id] }))}>{expanded ? 'Hide' : 'Read more'}</button>
                  </div>
                )}
              </div>
            </div>
